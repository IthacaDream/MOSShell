...
2. **Auto-wrap a plain module**: when no `__channel__.py` exists, exported
   callables from the module are registered as commands.
3. **Provide an explicit `channel_file`**: manually specify a `.py` file that
   defines `__channel__`.
4. **Wrap a standard library module**: for example, exposing `math` as a channel
   and calling its functions.
5. **Load from a script path (`.py`)**: provide a concrete python file path like
