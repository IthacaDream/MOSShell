...
@pytest.mark.asyncio
async def test_put_without_locator_generates_one(storage):
    img = _red_image()
    ...
    assert locator
    assert len(locator) == 12  # uuid.hex[:12]


@pytest.mark.asyncio
async def test_put_with_caller_locator_respects_it(storage):
    img = _red_image()
    meta = LocalImageMeta(locator="my-red", description="红图")
    item = _InMemoryImageItem(meta, img)

    locator = await storage.put(item)
    assert locator == "my-red"
...
async def test_put_updates_meta_dimensions_and_format(storage):
    ...
    stored = await storage.get(locator)
...
async def test_put_overwrite_updates_existing(storage):
    ...
    meta = LocalImageMeta(locator="overwrite", description="first")
    await storage.put(_InMemoryImageItem(meta, img1))

    meta2 = LocalImageMeta(locator="overwrite", description="second")
    await storage.put(_InMemoryImageItem(meta2, img2))

    metas = await storage.list_metas()
    assert len(metas) == 1
...
async def test_list_metas_empty(storage):
    assert await storage.list_metas() == []


@pytest.mark.asyncio
async def test_list_metas_all(storage):
    for i in range(3):
        meta = LocalImageMeta(locator=f"img-{i}", description=f"图{i}")
...
async def test_list_metas_with_query_matches_description(storage):
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="a", description="沙滩日落", tags=[]), _red_image()))
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="b", description="山顶日出", tags=[]), _red_image()))

    metas = await storage.list_metas(query="日落")
    assert len(metas) == 1
    assert metas[0].locator == "a"


@pytest.mark.asyncio
async def test_list_metas_with_query_matches_tags(storage):
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="a", description="", tags=["beach", "sunset"]), _red_image()))
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="b", description="", tags=["mountain"]), _red_image()))
...
async def test_list_metas_query_case_insensitive(storage):
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="x", description="SUNSET Beach"), _red_image()))
...
async def test_list_metas_respects_limit(storage):
    for i in range(10):
        await storage.put(_InMemoryImageItem(
            LocalImageMeta(locator=f"img-{i}", description=""), _red_image()))
...
async def test_get_existing(storage):
    ...
    meta = LocalImageMeta(locator="avatar", description="头像")
    await storage.put(_InMemoryImageItem(meta, img))

    item = await storage.get("avatar")
    assert item is not None
    assert item.meta.locator == "avatar"
...
async def test_delete_existing(storage):
    await storage.put(_InMemoryImageItem(
        LocalImageMeta(locator="del-me", description=""), _red_image()))
...
def test_as_content_returns_json():
    meta = LocalImageMeta(locator="test", description="测试", tags=["a", "b"])
    content = meta.as_content()
    assert '"locator":"test"' in content
