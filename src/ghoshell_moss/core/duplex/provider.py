...
from ghoshell_moss.core.concepts.errors import FatalError, CommandErrorCode, CommandError
from ghoshell_moss.core.concepts.command import CommandTask, BaseCommandTask
...
class DuplexChannelProvider(ChannelProvider):
    ...
    async def arun(self, channel: Channel) -> None:
        if self._starting:
            self.logger.info("DuplexChannelProvider[cls=%s] already started, channel=%s", self.__class__.__name__,
                             channel.name())
            return
        ...
        try:
            ...
            self.logger.info("DuplexChannelProvider[cls=%s] started, channel=%s", self.__class__.__name__,
                             channel.name())
    ...
    async def _clear_running_status(self) -> None:
        ...
        for val in done:
            if isinstance(val, Exception):
                self.logger.error("clear channel error %s", val)
