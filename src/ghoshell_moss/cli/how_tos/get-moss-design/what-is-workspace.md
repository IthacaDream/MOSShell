...
```bash
moss workspace where          # 看你当前的 workspace 在哪
```
...
```bash
# 列出实际的 workspace
ls -la $(moss workspace where 2>/dev/null | grep "Expect Root" | cut -d'|' -f3 | xargs)
```
...
```bash
moss codex get-source ghoshell_moss.host.moss_runtime:MossRuntimeImpl
```
...
# 非交互式（AI / 脚本）
moss workspace init --cwd -y      # 当前目录，跳过确认
moss workspace init /path -y      # 指定路径，跳过确认

# 复制环境变量模板
moss workspace copy-env
moss workspace copy-env --force   # 覆盖已有 .env
...
1. `moss workspace copy-env` — 创建 .env 文件
...
读者按照本文档操作，应该能够：
1. 用 `moss workspace where` 确认当前 workspace 位置
...
5. 用 `moss workspace init --cwd -y` 创建一个新的 workspace
