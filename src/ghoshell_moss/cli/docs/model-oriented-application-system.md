...
```
moss apps init ai_tools/calc          # 1. 在磁盘上创建 App 脚手架
...
不写则用默认值。`moss apps init` 会自动刷新 AppStore 的发现缓存。
...
- `moss apps init` 创建测试 App（greeter, ping_test, calc）
- 通过 MCP 启动、调试、发现 bug
...
```bash
# App 创建与管理
moss apps init <group/name>        # 创建 App 脚手架
moss apps list                     # 发现所有 App，含运行状态
