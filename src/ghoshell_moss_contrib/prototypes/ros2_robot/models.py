...
__all__ = [
    ...
    'Pose', 'Transition', 'Animation',
    'PoseTransition', 'PoseAnimation',
    'RobotInfo',
]
...
class Joint(BaseModel):
    ...
    origin_name: str = Field(
        default="",
        description="底层机器人系统里的关节名称. 和底层系统对齐, 并非必要. "
    )
    ...
    value_parser: str = Field(
        default="",
        description="将参数 value 转换成轨迹值 (通常是弧度) 的算法. 空表示默认. "
    )
...
class Trajectory(BaseModel):
    ...
    joint_names: List[str] = Field(
        ...
        description="关节的名称, 与每个轨迹点的位置值一致. "
    )
...
class Transition(BaseModel):
    ...
    positions: Dict[str, float] = Field(
        ...
        description="指定运行的姿态描述. 如果 pose name 存在的话, 则不需要这个参数."
    )
...
class PoseAnimation(BaseModel):
    ...
    transitions: List[PoseTransition] = Field(
        default_factory=list,
        description="动画的过度片段"
    )
...
class RobotInfo(BaseModel):
    ...
    controllers: Dict[str, Controller] = Field(
        default_factory=dict,
        description="可并行控制组件的描述"
    )
    default_pose_name: str = Field(
        default="",
        description="默认的 pose 名称",
    )
    poses: Dict[str, Pose] = Field(
        default_factory=dict,
        description="已经存储的所有位姿"
    )
    animations: Dict[str, Animation] = Field(
        default_factory=dict,
        description="机器人已经存储的所有的运动轨迹动画. 全身性的."
    )
