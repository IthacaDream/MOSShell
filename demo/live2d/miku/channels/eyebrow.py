...
async def _smooth_move_eyebrow(model: live2d.LAppModel, param_x, param_y, param_angle, target_x, target_y, target_angle, speed):
    ...
    while time.time() - start_time < move_duration:
        ...
        print(f"current_frame_x: {current_frame_x}, current_frame_y: {current_frame_y}, current_frame_angle: {current_frame_angle}")
    ...
    print(f"Eyebrow moved to target: x={target_x}, y={target_y}, angle={target_angle}")
...
async def move(x: float = 0.0, y: float = 0.0, angle: float = 0.0, speed: float = 1.0):
    ...
    # 调用通用平滑移动函数
    return await _smooth_move_eyebrow(
        model=model,
