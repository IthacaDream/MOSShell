...
.venv/bin/moss --ai codex blueprint channel_builder
```
...
- `moss codex concepts`: 基于代码反射列出当前架构的核心概念, 包括
    - `concepts`: 实现 MOSShell 感知/反射/解释/调度/通信 的基础抽象设计.
...
- `moss workspace`: 创建和管理本地 workspace.
...
- `moss codex blueprint channel_builder`: 了解如何构建一个 python 驱动的能力.
- `moss codex blueprint matrix`: 了解如何构建一个应用, 使之接入 MOSS.

## Host

之所以用 `Host` 而不是 `Server` 来描述 moss 运行时体系, 因为它目前主要运行在本地而非云端. Host 目标包括:

1. 基于 workspace 的环境发现. 详见 `ghoshell_moss.core.blueprint.environment`. 自动加载 workspace 里的 source.
...
4. 环境中发现的能力, 自动通过 Matrix 抽象集成, 同时通过 Session 抽象建立跨进程的通讯体系.
    - matrix: `moss codex blueprint matrix`
    ...
    - session: `moss codex blueprint session`
...
6. Host 提供不同类型的 Runtime. 核心包括:
    - ShellRuntime: 管理 MOSS Shell 的运行 `moss codex concepts shell`
    - Mindflow: 管理 MOSS 架构的感知体系, 和三循环双工状态仲裁 `moss codex blueprint mindflow`
